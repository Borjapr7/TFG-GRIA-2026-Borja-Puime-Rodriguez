...
# Wrapper del modelo para CLS Y CKA
class ModelForCKA(nn.Module):
    def __init__(self, model_name, cls_token_id):
        super().__init__()
        ...
        # Guardamos el ID del token que queremos buscar
    ...
    def forward(self, input_ids):
        ...
        # Buscamos dinámicamente la posición del token [CLS] en cada frase del batch
        cls_positions = (input_ids == self.cls_token_id).int().argmax(dim=1)
        batch_indices = torch.arange(input_ids.size(0), device=input_ids.device)
        
        # Iteramos por las 13 capas de salida
        for i, state in enumerate(outputs.hidden_states):
            # Extraemos el vector de la posición del [CLS]
            cls_token_embedding = state[batch_indices, cls_positions, :] 
            
            # Añadimos ruido infinitesimal en la capa 0 para evitar que la 
            # varianza sea exactamente 0 y produzca NaNs en CKA
            if i == 0:
                cls_token_embedding = cls_token_embedding + torch.randn_like(cls_token_embedding) * 1e-2
                
            # Lo pasamos por nuestra capa chivato para que torch-cka lo capture
            self.cka_layers[i](cls_token_embedding)
            
        return None # torch-cka captura por los "hooks"
...
# Tokenizamos usando el tokenizador base para obtener el cls_token_id
print("Tokenizando oraciones.")
tokenizer = AutoTokenizer.from_pretrained("bert-base-uncased")
inputs = tokenizer(oraciones, padding=True, truncation=True, return_tensors="pt")
cls_id = tokenizer.cls_token_id

# DataLoader estándar de PyTorch
dataset = TensorDataset(inputs.input_ids, torch.zeros(len(oraciones))) 
dataloader = DataLoader(dataset, batch_size=16, shuffle=False)

# Inicializamos nuestros wrappers pasándole el cls_id
print("\nCargando modelos.")
modelo_base = ModelForCKA("bert-base-uncased", cls_id).to(device).eval()
modelo_dominio = ModelForCKA("citiusLTL/SHBERT", cls_id).to(device).eval()

# Definimos los nombres de las capas a las que torch-cka debe engancharse
layer_names = [f"cka_layers.{i}" for i in range(13)]

# CKA INTRAMODELO
print("\nCalculando CKA Intramodelo (SHBERT vs SHBERT)")
cka_intra_obj = CKA(
    modelo_dominio, modelo_dominio,
    model1_name="SHBERT", model2_name="SHBERT",
...
# CKA INTERMODELO
print("Calculando CKA Intermodelo (BERT Base vs SHBERT).")
cka_inter_obj = CKA(
    modelo_base, modelo_dominio,
    model1_name="BERT_Base", model2_name="SHBERT",
    model1_layers=layer_names, model2_layers=layer_names,
    device=device
)
...
plt.savefig("cka_resultados_combinados_cls.png", dpi=300, bbox_inches='tight')
