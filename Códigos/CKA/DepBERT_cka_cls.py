...
modelo_dominio = ModelForCKA("citiusLTL/DepBERT", cls_id).to(device).eval()

# Definimos los nombres de las capas a las que torch-cka debe engancharse
layer_names = [f"cka_layers.{i}" for i in range(13)]

# CKA INTRAMODELO
print("\nCalculando CKA Intramodelo (DepBERT vs DepBERT)")
cka_intra_obj = CKA(
    modelo_dominio, modelo_dominio,
    model1_name="DepBERT", model2_name="DepBERT",
...
# CKA INTERMODELO
print("Calculando CKA Intermodelo (BERT Base vs DepBERT).")
cka_inter_obj = CKA(
    modelo_base, modelo_dominio,
    model1_name="BERT_Base", model2_name="DepBERT",
    model1_layers=layer_names, model2_layers=layer_names,
    device=device
)
