...
class ModelForCKA(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        for i, state in enumerate(outputs.hidden_states):
            ...
            mask_token = state[batch_indices, mask_positions, :] 
            
            # Ruido ajustado a 1e-8 para que funcione bien en float32 (Apple Silicon)
            ruido = torch.randn_like(mask_token) * 1e-8
            mask_token_estable = mask_token + ruido
            
            # Pasamos el token estabilizado a nuestra capa chivato para que torch-cka lo capture
            self.cka_layers[i](mask_token_estable)
...
# Inicializamos nuestros wrappers pasándole el mask_id (sin .double())
print("\nCargando modelos.")
modelo_base = ModelForCKA("bert-base-uncased", mask_id).to(device).eval()
modelo_dominio = ModelForCKA("citiusLTL/SHBERT", mask_id).to(device).eval()
...
plt.savefig("cka_resultados_combinados_sh_mask.png", dpi=300, bbox_inches='tight')
