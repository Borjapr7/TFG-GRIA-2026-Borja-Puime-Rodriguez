...
# Wrapper del modelo para MASK Y CKA
class ModelForCKA(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        for i, state in enumerate(outputs.hidden_states):
            # Extraemos el vector de la posición del [MASK]
...
# Tokenizamos usando el tokenizador base para obtener el mask_token_id
...
# Inicializamos nuestros wrappers pasándole el mask_id
print("\nCargando modelos.")
modelo_base = ModelForCKA("bert-base-uncased", mask_id).to(device).eval()
modelo_dominio = ModelForCKA("citiusLTL/AnorBERT", mask_id).to(device).eval()
...
plt.savefig("cka_resultados_combinados_mask.png", dpi=300, bbox_inches='tight')
